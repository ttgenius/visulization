#!/usr/bin/env python
#TODO 1.FIX bug (seen screenshot bug.png) 2.Ask victor the chunk of weird road in 7_6_dec under qcore/images
import subprocess
import gmt
import os
import shutil
...
def cp_and_rename(pattern, dir):
    ...
    for item in glob.glob1(dir,pattern):
        ...
        print("s",s)
        shutil.copy(s, os.path.join(dir, "{}_cp_{}_2".format(item[:7],item[7:])))
...
def draw_road(file,input_dir, p, even):
    """Second verison of drawing base path and vehicles, can be potentially optimized?
       file: input gmt file to read
       p:GMTPLOT object
       even: draw at even or odd position
    """
...
def create_pngs(in_name,input_dir,outdir,even=True):
    """create GMTPlot object"""
    postfix = '_even' if even else '_odd'
    if '/' not in outdir:
        outdir+='/'
    if '/' not in input_dir:
        input_dir+='/'
    out_name = outdir + in_name[:-4]+postfix+'.ps'
    p = gmt.GMTPlot(pspath=out_name)
    p.spacial('M', REGION, sizing=7)
    ...
    draw_road(in_name,input_dir, p, even)
    # read_all('data/',p)
    # p.image(CHCH[0],CHCH[1],'/home/yzh231/airport.png', width='0.4i',align='C')
    p.finalise()
    p.png(out_dir=outdir)

def main():
    outdir='/usr/local/qcore_data/yzh231/qcore/melodying/images'
    # input_dir = '/usr/local/qcore_data/yzh231/qcore/melodying/data/modified_road_data/'
    # for file in glob.glob1('/usr/local/qcore_data/yzh231/qcore/melodying/data/modified_road_data/','*16*'):
    #     create_pngs(file,input_dir,outdir, even=True)
    #     create_pngs(file, input_dir,outdir,even=False)
    # cp_and_rename('*',outdir)
    # custom_make_movie('images*', output_name='all_roads')
    custom_make_movie("*im*.png", "test_movie.mov")
    # # print(CHCH[0])
    ...
    # p.spacial('M', REGION, sizing = 7)y
    # p.basemap(water='black',res = 'f', topo = None)
    # p.sites(['Wellington,RM','Christchurch'])
    # #
    # file = '/usr/local/qcore_data/yzh231/qcore/melodying/data/3_15_nov_2016.gmt'
    ...
    # p.finalise()
    # p.png(background='darkblue')
    #custom_make_movie("3_15*.png","3_15.mp4")
    # read_all('/usr/local/qcore_data/yzh231/qcore/melodying/pys/test')
